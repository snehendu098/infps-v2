...
export default function HeroSection() {
  ...
  const showParticleLogo = settings.particleLogo?.enabled !== false;

  // Hero content from Sanity with fallbacks
  const heroContent = settings.heroSection || {};
  const headingLine1 = heroContent.headingLine1 || 'Transform, Innovate';
  const headingLine2 = heroContent.headingLine2 || '& Scale Your Business';
  const description = heroContent.description || 'Leading provider of Data Center Management, Custom Software Development, Smart City Solutions, and comprehensive Digital Services';
  const primaryButtonText = heroContent.primaryButtonText || 'Start Your Project';
  const primaryButtonLink = heroContent.primaryButtonLink || '/contact';
  const secondaryButtonText = heroContent.secondaryButtonText || 'Explore Services';
  const secondaryButtonLink = heroContent.secondaryButtonLink || '/services';
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 md:px-6 lg:px-8 text-center py-8 md:py-0">
        ...
        {showParticleLogo && (
          <div
            ...
            style={{
              width: '100%',
              maxWidth: '1200px',
              height: 'clamp(200px, 35vh, 640px)',
              margin: '0 auto'
            }}
          >
            <ParticleLogoCanvas
              count={settings.particleLogo?.particleCount}
              colorA={settings.particleLogo?.colorA}
              colorB={settings.particleLogo?.colorB}
              forceRadius={settings.particleLogo?.forceRadius}
              shadowBlur={settings.particleLogo?.shadowBlur}
            />
          </div>
        )}
        ...
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-extrabold leading-tight mb-6 sm:mb-8 md:mb-6 slide-in-left px-2">
          <span className="gradient-text">{headingLine1}</span>
          <br />
